package ru.alexsergeev.cryptoviewer.presentation.ui.components.logotypes

import androidx.compose.foundation.layout.Box
import androidx.compose.foundation.layout.height
import androidx.compose.foundation.layout.size
import androidx.compose.foundation.layout.width
import androidx.compose.material3.Icon
import androidx.compose.runtime.Composable
import androidx.compose.ui.Alignment
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.res.painterResource
import androidx.compose.ui.unit.dp
import ru.alexsergeev.cryptoviewer.presentation.R
import ru.alexsergeev.cryptoviewer.presentation.theme.CryptoTheme

@Composable
internal fun AdaLogo() {
    Box(contentAlignment = Alignment.Center) {
        Icon(
            modifier = Modifier.size(40.dp),
            painter = painterResource(id = R.drawable.ada_base),
            contentDescription = "ada_base",
            tint = CryptoTheme.colors.adaLogoColor
        )
        Icon(
            painterResource(id = R.drawable.ada_logo),
            "ada_logo",
            tint = Color.White
        )
    }
}

@Composable
internal fun AdaLogoBig() {
    Box(contentAlignment = Alignment.Center) {
        Icon(
            modifier = Modifier.size(90.dp),
            painter = painterResource(id = R.drawable.ada_base),
            contentDescription = "ada_base",
            tint = CryptoTheme.colors.adaLogoColor
        )
        Icon(
            modifier = Modifier.height(58.dp).width(62.dp),
            painter = painterResource(id = R.drawable.ada_logo),
            contentDescription = "ada_logo",
            tint = Color.White
        )
    }
}